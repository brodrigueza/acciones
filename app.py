...
def obtener_datos_mercado(tickers):
    ...
    for t in tickers:
        try:
            ...
            # Extraer metadata de sector
            try:
                info = stock.info
                sectores[t] = info.get('sector', 'No Clasificado')
            except:
                sectores[t] = 'No Clasificado'
    ...
    return precios_actuales, historial_dividendos, historial_precios, sectores
    ...
    if es_nacional or divs_validos.empty or serie_precios.empty:
        ...
        for fecha_div, monto_div in divs_validos.items():
            ...
            if pd.notna(precio_fecha) and precio_fecha > 0:
                # 15% de impuesto para Chile (retienes el 85% del dividendo)
                impuesto_usa = 0.85 
...
st.markdown("Las acciones internacionales reinvierten dividendos automáticamente (15% tax). Los dividendos nacionales se asumen reinvertidos manualmente.")
...
if archivo_excel:
    try:
        ...
        # Corrección si las columnas Fecha y Precio vienen invertidas por error
        ...
        precios_dict, dividendos_dict, precios_hist_dict, sectores_dict = obtener_datos_mercado(tickers_unicos)

        df_transacciones["Precio_Actual"] = df_transacciones["Ticker"].map(precios_dict)
        df_transacciones["Sector"] = df_transacciones["Ticker"].map(sectores_dict)
        ...
        # Agrupación por Ticker para el consolidado
        ...
        # Capital real aportado = Todo lo comprado menos lo financiado con dividendos
        ...
        # --- SECCIÓN 2: GRÁFICOS (3 Columnas) ---
        st.subheader("Distribución Patrimonial")
        col_torta1, col_torta2, col_torta3 = st.columns(3)
        
        df_grafico = df_portafolio[['Ticker', 'Valor_Posicion_CLP', 'Sector']].copy()
        ...
        with col_torta3:
            df_sectores = df_grafico.groupby("Sector", as_index=False)["Valor"].sum()
            fig_sector = px.pie(df_sectores, values='Valor', names='Sector', hole=0.4, title="Exposición por Sector",
                                color_discrete_sequence=px.colors.qualitative.Prism)
            fig_sector.update_traces(textposition='inside', textinfo='percent+label', showlegend=False)
        ...
        # Aplicamos formato dejando la columna sector sin tocar
